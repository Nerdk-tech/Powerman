...
export default async function HomePage() {
  ...
  const featured = [...products].sort((a, b) => Number(b.isFeatured) - Number(a.isFeatured)).slice(0, 12);

  return (
    <>
      
      <section className="bg-navy">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-4 px-4 py-6 text-center sm:py-8 lg:flex-row lg:items-center lg:justify-between lg:text-left">
          <div>
            <h1 className="text-2xl font-extrabold leading-tight tracking-tight text-white sm:text-3xl">
              Reliable Power Starts With the <span className="text-volt">Right Battery.</span>
            </h1>
            <p className="mt-1.5 text-sm text-navy-100">
              Order on WhatsApp or call {SITE_CONFIG.phone} — delivery available.
            </p>
          </div>
          <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
            <Link
              href="/shop"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-volt px-6 py-3 text-sm font-bold text-navy shadow-glow transition-all hover:-translate-y-0.5 hover:bg-volt-400"
            >
              All Batteries <ArrowRight size={15} />
            </Link>
            <a
              href={buildWhatsAppLink()}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 rounded-full border border-white/30 bg-white/5 px-6 py-3 text-sm font-bold text-white backdrop-blur-sm transition-colors hover:border-volt hover:text-volt"
            >
      ...
      <section className="bg-navy-50 bg-grid-pattern py-8 sm:py-10">
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-volt-600">Available now</p>
              <h2 className="mt-2 text-3xl font-extrabold text-navy sm:text-4xl">Our Batteries</h2>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:py-20">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-volt-600">Why ENERGY MAN</p>
          <h2 className="mt-2 text-3xl font-extrabold text-navy sm:text-4xl">Why Choose ENERGY MAN?</h2>
        </div>
