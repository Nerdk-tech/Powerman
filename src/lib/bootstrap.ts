...
const DDL: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS "SiteContent" (
    ...
    "email" TEXT NOT NULL DEFAULT 'info@energymanbatteries.com',
  ...
  if (adminCount === 0) {
    try {
      await client.adminUser.create({
        data: { username: 'admin', passwordHash: bcrypt.hashSync('EnergyMan2026!', 10) },
      });
    } catch (e) {
      if (!isUniqueViolation(e)) throw e;
    }
...
async function applyRebrand(client: PrismaClient): Promise<void> {
  const productCols = ['name', 'brand', 'manufacturer', 'description', 'compatibilityNotes'];
  for (const col of productCols) {
    await client.$executeRawUnsafe(
      `UPDATE "Product" SET "${col}" = REPLACE(REPLACE("${col}", 'POWERMAN', 'ENERGY MAN'), 'Powerman', 'Energy Man') WHERE "${col}" ILIKE '%powerman%'`
    );
  }
  await client.$executeRawUnsafe(
    `UPDATE "Product" SET "slug" = REPLACE("slug", 'powerman', 'energy-man') WHERE "slug" ILIKE '%powerman%'`
  );
  await client.$executeRawUnsafe(
    `UPDATE "Product" SET "model" = REPLACE("model", 'PM-', 'EM-') WHERE "model" LIKE 'PM-%'`
  );
  await client.$executeRawUnsafe(
    `UPDATE "Product" SET "sku" = REPLACE("sku", 'PM-', 'EM-') WHERE "sku" LIKE 'PM-%'`
  );
  const contentCols = ['aboutText', 'missionText', 'visionText', 'heroHeadline', 'heroSubheadline', 'promoBannerText'];
  for (const col of contentCols) {
    await client.$executeRawUnsafe(
      `UPDATE "SiteContent" SET "${col}" = REPLACE(REPLACE("${col}", 'POWERMAN', 'ENERGY MAN'), 'Powerman', 'Energy Man') WHERE "${col}" ILIKE '%powerman%'`
    );
  }
  await client.$executeRawUnsafe(
    `UPDATE "Enquiry" SET "productName" = REPLACE(REPLACE("productName", 'POWERMAN', 'ENERGY MAN'), 'Powerman', 'Energy Man') WHERE "productName" ILIKE '%powerman%'`
  );
  const admin = await client.adminUser.findUnique({ where: { username: 'admin' } });
  if (admin && bcrypt.compareSync('PowerMan2026!', admin.passwordHash)) {
    await client.adminUser.update({
      where: { id: admin.id },
      data: { passwordHash: bcrypt.hashSync('EnergyMan2026!', 10) },
    });
  }
}
